...
public class CritDBExtension extends AbstractSimExtension {
	...
	public void run(String page) {
		...
		if (action == null || action.equals("")) {
			if (StringUtils.isInteger(request.getParameter("race"))) {
				request.setAttribute("critdb_creatures", Creature.getCreatures(request));
				setPath("/critdb.jsp");
			} else {
				setPath("/main.jsp");
			}
		} 
		else if (action.equals("search")) doSearch();
		else if (action.equals("critcalc")) doCritCalc();
		...
		if (post) {
			try {
				...
				request.setAttribute("critdb_DY",
						be.lacerta.cq2.battlecalc.objects.Creature.convertStrength(
								Integer.parseInt(request.getParameter("DX")), 
								Integer.parseInt(request.getParameter("LX")),
								Integer.parseInt(request.getParameter("LY"))));
				
				request.setAttribute("critdb_HY",
						be.lacerta.cq2.battlecalc.objects.Creature.convertStrength(
								Integer.parseInt(request.getParameter("HX")), 
								Integer.parseInt(request.getParameter("LX")),
								Integer.parseInt(request.getParameter("LY"))));
	...
	private void doCritList() {
		...
		for (int i=0;i<classes.length; i++) {
			List<Creature> crits = Creature.getCreatures(classes[i],("show").equals(request.getParameter("all")));
	...
	private void doItemList() {
		...
		for (int i=0;i<classes.length; i++) {
			List<Item> items = Item.getItems(classes[i],("show").equals(request.getParameter("all")));
	...
	private void doCostCalc() {
		if (post) {
			...
			while (i<6 && !getString("ammy"+i).equals("")) {
				...
				Amulet amu = CreatureFinder.findAmulet(request.getParameter("ammy"+i).trim());
				if (amu!=null) amuPrices.put(amu, amount);
				i++;
			}
			...
			if (amuPrices.size() > 0) request.setAttribute("costcalc_amuprices", amuPrices);
			if (enchantPrices.size() > 0) request.setAttribute("costcalc_enchantprices", enchantPrices);
		}
		request.setAttribute("costcalc_MIenchanters", MageSkill.loadBySkill(Skill.loadByText("Major Immunity")));
		request.setAttribute("costcalc_MBenchanters", MageSkill.loadBySkill(Skill.loadByText("Major Berserk")));
	...
	private void doSummonChanceCalc() {
		...
		if (post) {
			...
			request.setAttribute("critdb_chance", CQ2Functions.calcSummonChance(mageSkill,critSkill));
		}
		setPath("/critcalc.jsp");
	}
